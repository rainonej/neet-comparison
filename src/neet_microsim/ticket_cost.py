"""Ticket-cost trajectories: cash, opportunity cost, years, and accessible-seat odds.

Uses CMSE school-tutoring floors, Rajan Committee coaching anchors, PLFS no-college
wages for opportunity cost, and score-privilege accessible-seat rates. Not causal LATE
estimates. Birth-year / Class XII year of admits is still missing nationally; TN
current-year vs other-than-current-year composition is the best age/attempt proxy in hand.
"""
...
RAJAN_LONG_TERM_MID_INR = 250_000.0  # mid of reported long-term package band (~2.5–4.5L)
...
def build_trajectories(processed: Path | None = None) -> list[TicketTrajectory]:
    ...
    fresh_coaching = CostBand(low=0.0, mid=cmse["p50"], high=cmse["p90"])
    fresh_exam = CostBand(
    ...
    # Two failed attempts + third sit: ~2 full drop years after XII.
    drop_coaching = CostBand(
        low=2.0 * cmse["p90"],  # school-tutoring floor (weak for residential droppers)
        mid=2.0 * RAJAN_LONG_TERM_MID_INR,
        high=RAJAN_REPEATER_COACHING_EXCLUSIVE_INR,
    )
    drop_exam = CostBand(
        low=2.0 * NEET_EXAM_FEE_INR,
        mid=3.0 * NEET_EXAM_FEE_INR + 15_000.0,
        high=3.0 * NEET_EXAM_FEE_INR + 40_000.0,
    )
    drop_lodge = CostBand(
        low=0.0,
        mid=2.0 * 80_000.0,  # modest hostel/relocation for two years
        high=2.0 * 180_000.0,
    )
    drop_opp = CostBand(
        low=1.0 * annual_opp,  # partial / informal work possible
        mid=2.0 * annual_opp,
        high=2.0 * annual_opp * 1.25,
    )
    drop_total = _band_sum(drop_coaching, drop_exam, drop_lodge, drop_opp)

    # Affluent path: same calendar misery can buy higher odds.
    ...
    pct = lambda p: f"{100.0 * p:.1f}%"  # noqa: E731
    ...
    return [
        TicketTrajectory(
            ...
            punch_line=(
                f"You paid almost nothing to sit right out of high school "
                f"(cash outlay mid ≈ {inr_lakh(fresh_total.mid)}). "
                f"Your chance of an accessible MBBS seat your family can cash is "
                f"{pct(fresh_access)} — not a rounding error of 0.01%, and still a brutal lottery."
            ),
            sources=[
                "score_inequality_story.json unilateral tamil_cant_afford_nonmetro_none",
                "CMSE 2025 class_xii coaching spend (school frame)",
                "NTA exam fee order-of-magnitude",
            ],
            caveats=[
                "Accessible seat ≠ qualify; private offer you cannot pay does not count.",
                "CMSE is school tutoring, not NEET residential coaching.",
                "Rate is synthetic-stratum model output, not a published individual probability.",
            ],
        ),
        TicketTrajectory(
            ...
            psych_burden=(
                ...
                "is documented in convenience samples (e.g. Alagappa / district surveys). "
                "Not an individual clinical risk score."
            ),
            delayed_life_note=(
                "Two years of delayed college/work entry; peers may already be earning or enrolled."
            ),
            punch_line=(
                f"After ~2 years of studying, failed attempts, tutoring/lodging, and "
                f"family opportunity cost (economic mid ≈ {inr_lakh(drop_total.mid)}; "
                f"high coaching path ≈ {inr_lakh(drop_total.high)}), "
                f"your chance of an accessible seat your family can afford is still only "
                f"{pct(modest_access)} — a thin upgrade bought with years and money."
            ),
            sources=[
                "score_inequality_story.json unilateral tamil_cant_afford_nonmetro_modest",
                "Rajan Committee: ₹10L exclusive repeater coaching; long-term fee bands; avg ₹95,033",
                "CMSE 2025 class_xii p90 as school-tutoring floor",
                "PLFS 2025 no_college monthly median × 24 months opportunity cost",
            ],
            caveats=[
                "Rajan ₹10L is a TN narrative high for exclusive coaching — not a national mean.",
                "Opportunity cost uses no_college wage median as a transparent counterfactual.",
                "Modest prep does not unlock private seats the family still cannot pay.",
            ],
        ),
        TicketTrajectory(
            ...
            punch_line=(
                f"Same two-year ticket price in calendar time (economic mid ≈ {inr_lakh(aff_total.mid)}), "
                f"but privilege raises accessible-seat chance to {pct(top_access)} — "
                f"still a minority outcome, now with a privatized safety net."
            ),
            sources=[
            ...
            caveats=[
                ...
                "Can-pay private converts more score ranks into cashable seats.",
            ],
        ),
    ]
...
def admitted_composition_story(processed: Path | None = None) -> dict[str, Any]:
    """TN current-year vs repeater shares — best proxy for 'how old were the winners'."""
...
def write_ticket_cost_artifacts(
    ...
    summary = {
        ...
        "warnings": [
            "Do not quote 0.01% unless a stratum model actually produces it; use published p_accessible.",
            "TN admitted current-year share is not national birth-year microdata.",
